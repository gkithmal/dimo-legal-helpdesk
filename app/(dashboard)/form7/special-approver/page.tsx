...
import {
  ...
  Paperclip, CheckCircle2, X, AlertCircle, Loader2,
  Send, ArrowLeft, FileText,
  ThumbsUp, ThumbsDown,
} from 'lucide-react';
...
function ReadField({ label, value, multiline = false }: { label: string; value: string; multiline?: boolean }) {
  return (
    <div>
      ...
      <div className={`w-full px-3.5 py-2.5 rounded-lg border border-slate-200 bg-slate-50 text-sm text-slate-700 min-h-[40px] ${multiline ? 'whitespace-pre-wrap leading-relaxed' : ''}`}>
...
function SpecialApproverForm7Content() {
  ...
  // f7* fields
  const [f7AgreementRefNo,          setF7AgreementRefNo]          = useState('');
  const [f7AgreementDate,           setF7AgreementDate]           = useState('');
  const [f7InitiatorContact,        setF7InitiatorContact]        = useState('');
  const [f7AssessmentAddress,       setF7AssessmentAddress]       = useState('');
  const [f7OwnerNames,              setF7OwnerNames]              = useState('');
  const [f7EffectiveTerminationDate,setF7EffectiveTerminationDate]= useState('');
  const [f7EarlyTerminationCharges, setF7EarlyTerminationCharges] = useState('');
  const [f7RefundableDeposit,       setF7RefundableDeposit]       = useState('');
  const [f7AdvanceRentals,          setF7AdvanceRentals]          = useState('');
  const [f7PaymentDate1,            setF7PaymentDate1]            = useState('');
  const [f7FacilityPayments,        setF7FacilityPayments]        = useState('');
  const [f7PaymentDate2,            setF7PaymentDate2]            = useState('');
  const [f7Deductions,              setF7Deductions]              = useState('');
  const [f7Penalty,                 setF7Penalty]                 = useState('');
  const [f7AmountDueByDimo,         setF7AmountDueByDimo]         = useState('');
  const [f7BalanceToRecover,        setF7BalanceToRecover]        = useState('');
  const [f7DateInformedToLessee,    setF7DateInformedToLessee]    = useState('');
  ...
  useEffect(() => {
    ...
    fetch(`/api/submissions/${submissionId}`)
      ...
      .then(d => {
        ...
        setF7AgreementRefNo(s.f7AgreementRefNo || '');
        setF7AgreementDate(s.f7AgreementDate || '');
        setF7InitiatorContact(s.f7InitiatorContact || '');
        setF7AssessmentAddress(s.f7AssessmentAddress || '');
        setF7OwnerNames(s.f7OwnerNames || '');
        setF7EffectiveTerminationDate(s.f7EffectiveTerminationDate || '');
        setF7EarlyTerminationCharges(s.f7EarlyTerminationCharges || '');
        setF7RefundableDeposit(s.f7RefundableDeposit || '');
        setF7AdvanceRentals(s.f7AdvanceRentals || '');
        setF7PaymentDate1(s.f7PaymentDate1 || '');
        setF7FacilityPayments(s.f7FacilityPayments || '');
        setF7PaymentDate2(s.f7PaymentDate2 || '');
        setF7Deductions(s.f7Deductions || '');
        setF7Penalty(s.f7Penalty || '');
        setF7AmountDueByDimo(s.f7AmountDueByDimo || '');
        setF7BalanceToRecover(s.f7BalanceToRecover || '');
        setF7DateInformedToLessee(s.f7DateInformedToLessee || '');

        if (s.approvals?.length) {
          s.approvals.forEach((a: any) => {
            if (a.role === 'BUM')             setBum(a.approverName || '');
            if (a.role === 'GENERAL_MANAGER') setGm(a.approverName  || '');
          });
        }
  ...
  const handleApprove = async () => {
    ...
    setIsActioning(true); setApiError('');
    try {
      const res = await fetch(`/api/submissions/${submissionId}/approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'APPROVED', role: 'SPECIAL_APPROVER',
          approverName: currentUserName, approverEmail: session?.user?.email, approverId: session?.user?.id,
        }),
      });
  ...
  const handleReject = async () => {
    ...
    try {
      const res = await fetch(`/api/submissions/${submissionId}/approve`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'SENT_BACK', role: 'SPECIAL_APPROVER',
          approverName: currentUserName, approverEmail: session?.user?.email, approverId: session?.user?.id,
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          <div className="rounded-2xl overflow-hidden shadow-sm" style={{ background: 'linear-gradient(135deg, #1A438A 0%, #1e5aad 100%)' }}>
            <div className="flex items-center justify-between px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-white font-bold text-base leading-tight">Termination of agreements/lease agreements</h1>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm">
            ...
            <div className="px-6 py-6 space-y-5">
              ...
              <SectionDivider>Agreement Information</SectionDivider>
              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Agreement Reference No" value={f7AgreementRefNo} />
                <ReadField label="Agreement Date" value={f7AgreementDate} />
              </div>
              <ReadField label="Owner Names" value={f7OwnerNames} />

              <SectionDivider>Assessment Details</SectionDivider>
              <ReadField label="Assessment Address" value={f7AssessmentAddress} multiline />
              <ReadField label="Initiator Contact" value={f7InitiatorContact} />

              <SectionDivider>Termination Terms</SectionDivider>
              <ReadField label="Effective Termination Date" value={f7EffectiveTerminationDate} />
              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Early Termination Charges (Rs.)" value={f7EarlyTerminationCharges} />
                <ReadField label="Refundable Deposit (Rs.)" value={f7RefundableDeposit} />
              </div>

              <SectionDivider>Financial Settlement</SectionDivider>
              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Advance Rentals (Rs.)" value={f7AdvanceRentals} />
                <ReadField label="Payment Date" value={f7PaymentDate1} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Facility Payments (Rs.)" value={f7FacilityPayments} />
                <ReadField label="Payment Date" value={f7PaymentDate2} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Deductions (Rs.)" value={f7Deductions} />
                <ReadField label="Penalty (Rs.)" value={f7Penalty} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Amount Due By DIMO (Rs.)" value={f7AmountDueByDimo} />
                <ReadField label="Balance To Recover (Rs.)" value={f7BalanceToRecover} />
              </div>

              <SectionDivider>Key Dates</SectionDivider>
              <ReadField label="Date Informed to Lessee" value={f7DateInformedToLessee} />
            </div>
          </div>
        </div>
        ...
        <div className="w-[296px] flex-shrink-0 flex flex-col gap-4">
          ...
          <PanelSection title="Approvals">
            <div className="p-4 space-y-3.5">
              {[{ label: 'BUM', value: bum }, { label: 'General Manager', value: gm }].map(({ label, value }) => (
                <div key={label}>
                  <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">{label}</label>
          ...
          <div className="flex gap-2">
            <button onClick={() => router.push(ROUTES.SPECIAL_APPROVER_HOME)} disabled={isActioning}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col items-center text-center">
            ...
            <div className="flex gap-3 w-full">
              ...
              <button onClick={handleApprove} disabled={isActioning || hasActed}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleReject} disabled={isActioning || hasActed || !rejectReason.trim()}
                className="flex-1 py-2.5 rounded-xl font-bold text-sm text-white disabled:opacity-70"
                style={{ background: 'linear-gradient(135deg, #dc2626, #ef4444)' }}>
      ...
      {showSignOut && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 mx-4 w-full max-w-sm z-10">
            ...
            <h3 className="text-lg font-bold text-slate-800 mb-1 text-center">Sign Out</h3>
            <p className="text-sm text-slate-500 mb-5 text-center">Are you sure you want to sign out?</p>
            <div className="flex gap-3">
...
export default function SpecialApproverForm7Page() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="text-gray-600">Loading...</div></div>}>
      <SpecialApproverForm7Content />
    </Suspense>
  );
}
