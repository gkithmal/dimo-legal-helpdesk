...
function OfficialUseModal({ submissionNo, submissionId, onClose, onComplete, initialFields, readOnly = false }: {
  ...
  const REQUIRED_FIELDS = ['dateOfSearchNIPO', 'trademarkNo'];
  const FIELD_LABELS: Record<string, string> = {
    dateOfSearchNIPO: 'Date of Search at NIPO',
    trademarkNo: 'Trademark No.',
  };
  ...
  const saveToDb = async (extraData?: Record<string, unknown>) => {
    await fetch(`/api/submissions/${submissionId}`, {
      ...
      body: JSON.stringify({
        ouRegisteredDate: fields.dateOfSearchNIPO || null,
        ouLegalRefNumber: fields.trademarkNo || null,
        ouDateOfExecution: fields.datePublicationGazette || null,
        ouDateOfExpiration: fields.certificateIssuedDate || null,
        ouDirectorsExecuted1: fields.refusalHearingDate || null,
        ouDirectorsExecuted2: fields.dateSubmittingWritten || null,
        ouRemarks: fields.remarksLO || null,
  ...
  if (modal === 'jobSuccess') return (
    <SuccessModal title="Completed!" message="The trademark registration has been completed." submissionNo={submissionNo} onClose={() => { setModal(null); onClose(); }} />
  );
    ...
    <div className="min-h-screen flex bg-[#f0f4f9]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {showOfficialUse && <OfficialUseModal
        ...
        initialFields={{
          ...
          datePublicationGazette: (submission as any).ouDateOfExecution || '',
          certificateIssuedDate: (submission as any).ouDateOfExpiration || '',
          refusalHearingDate: (submission as any).ouDirectorsExecuted1 || '',
          dateSubmittingWritten: (submission as any).ouDirectorsExecuted2 || '',
          remarksLO: (submission as any).ouRemarks || '',
        }}
      />}
