...
import {
  ...
  LogOut, ArrowLeft, CheckCircle2, FileText, Clock, XCircle,
...
function ReassignModal({ currentOfficer, officers, onSave, onClose }: {
  currentOfficer: string; officers: { name: string; email: string }[]; onSave: (name: string, email: string) => void; onClose: () => void;
}) {
...
function LegalGMForm6PageContent() {
  ...
  const loadSubmission = useCallback(async () => {
    ...
    try {
      ...
      if (s.formId && s.formId !== 6) {
        if (s.formId && s.formId !== 6) { router.replace(`/form${s.formId}/legal-gm?id=${submissionId}`); return; };
        return;
      }
      setSubmission(s);
      ...
      const commentEntries = (s.comments || []).map((c: any, i: number) => ({
        id: 1000 + i,
        actor: c.authorName,
        role: c.authorRole === 'LEGAL_GM' ? 'Legal GM'
          : c.authorRole === 'LEGAL_OFFICER' ? 'Legal Officer'
          : c.authorRole,
        action: `Comment: "${c.text}"`,
        comment: undefined,
        timestamp: fmtDate(c.createdAt),
        _sortKey: new Date(c.createdAt).getTime(),
      }));

      const allEntries = [
        { id: 0, actor: 'System', role: 'System', action: 'Submission created', timestamp: fmtDate(s.createdAt), _sortKey: new Date(s.createdAt).getTime() },
        ...approvalEntries,
        ...commentEntries,
  ...
  useEffect(() => {
    fetch('/api/users?role=LEGAL_OFFICER')
      .then(r => r.json())
      .then(d => { if (d.success) setLegalOfficers(d.data.map((u: any) => ({ id: u.id, name: u.name || '', email: u.email }))); })
      .catch(() => {});
  }, []);
    ...
    fetch('/api/users')
      ...
      .then(data => {
        ...
        const officers = data.data
          .filter((u: { role: string; isActive: boolean }) => u.role === 'LEGAL_OFFICER' && u.isActive)
          .map((u: { name: string; email: string }) => ({ name: u.name || u.email, email: u.email }));
        setLegalOfficers(officers);
      })
      .catch(console.error);
  }, []);
    ...
    <div className="min-h-screen flex bg-[#f0f4f9]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <aside className="w-[68px] flex flex-col items-center py-5 gap-4 flex-shrink-0 sticky top-0 h-screen"
        ...
        <nav className="flex flex-col items-center gap-1 flex-1 w-full px-2">
          <NotificationBell />
          <button onClick={() => router.push(ROUTES.LEGAL_GM_HOME)} className="relative w-full h-10 rounded-xl flex items-center justify-center text-white/50 hover:bg-white/10 hover:text-white transition-all" title="Home">
          ...
          <div className="rounded-2xl overflow-hidden shadow-sm" style={{ background: 'linear-gradient(135deg, #1A438A 0%, #1e5aad 100%)' }}>
            <div className="flex items-center justify-between px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-white font-bold text-base">Registration of a Trademark</h1>
                  <p className="text-white/50 text-[11px] mt-0.5 font-mono">18/FM/1641/07/06</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div className="bg-white text-[#1A438A] font-bold text-sm px-4 py-2 rounded-xl">Form 6</div>
              </div>
            </div>
          </div>
            ...
            <div className="px-6 py-6 space-y-5">
              ...
              {(() => {
                try {
                  ...
                  return (
                    <>
                      <ReadField label="Name of the Applicant" value={scope.applicantName || ''} />
                    </>
                  );
                } catch { return null; }
              })()}
              ...
              {(() => {
                try {
                  ...
                  return (
                    <>
                      <ReadField label="Trademark &amp; Classes" value={scope.trademarkClass || ''} multiline />
                      <ReadField label="Artwork / Word" value={scope.artworkOrWord || ''} multiline />
                      <ReadField label="Remarks" value={scope.remarks || ''} multiline />
                    </>
                  );
                } catch {
                  return <ReadField label="Scope" value={submission.scopeOfAgreement} multiline />;
                }
              })()}
        ...
        <div className="w-[296px] flex-shrink-0 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            ...
            <div className="border-t border-slate-100">
              ...
              {stage === 'FINAL_APPROVAL' ? (
                <div className="p-3">
                  {(() => {
                    const draftDoc = submission.documents.find((d) => d.label === 'Draft Agreement' || d.type === 'DRAFT');
                    return draftDoc?.fileUrl ? (
                      <button onClick={() => window.open(draftDoc.fileUrl!, '_blank')}
                        className="flex items-center gap-2 w-full rounded-lg px-3 py-2.5 bg-[#EEF3F8] border border-[#1A438A]/20 hover:bg-[#dce8f3] transition-colors">
                        <Eye className="w-4 h-4 text-[#1A438A]" />
                        <span className="text-sm font-semibold text-[#1A438A]">Draft Agreement</span>
                      </button>
                    ) : (
                      <p className="text-[11px] text-slate-400 italic px-1">No draft uploaded yet</p>
                    );
                  })()}
                </div>
              ) : (
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { if (isInitial && !assignedOfficer.id && !assignedOfficer.name) { setApiError('A Legal Officer must be assigned before proceeding. Please assign one using the Reassign button.'); return; } setApiError(''); setShowConfirmAction('approve'); }} disabled={isActing || gmAlreadyActed}
  ...
  if (submissionId) {
    ...
    await fetch(`/api/submissions/${submissionId}`, {
      ...
      {showSuccess === 'approve' && <SuccessModal title={stage === 'FINAL_APPROVAL' ? 'Completed!' : 'Approved!'} message={stage === 'FINAL_APPROVAL' ? 'The trademark registration has been fully approved and completed.' : 'Request has been sent to the Legal Officer.'} submissionNo={submission.submissionNo} onClose={() => { setShowSuccess(null); router.push(ROUTES.LEGAL_GM_HOME); }} />}
      {showSuccess === 'sendback' && <SuccessModal title="Sent Back!" message="Trademark registration form has been sent back." submissionNo={submission.submissionNo} onClose={() => { setShowSuccess(null); router.push(ROUTES.LEGAL_GM_HOME); }} />}
      {showSuccess === 'cancel'   && <SuccessModal title="Rejected!" message="Trademark registration form has been rejected and cancelled." submissionNo={submission.submissionNo} onClose={() => { setShowSuccess(null); router.push(ROUTES.LEGAL_GM_HOME); }} />}
    </div>
  );
}
export default function LegalGMForm6Page() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="text-gray-600">Loading...</div></div>}>
      <LegalGMForm6PageContent />
    </Suspense>
  );
}