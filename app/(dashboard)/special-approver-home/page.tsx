  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        // Special approver tasks — submissions with PENDING_SPECIAL_APPROVER status
        const myApprovals = all.filter((s: any) =>
          s.status === 'PENDING_SPECIAL_APPROVER' ||
          (s.approvals?.some((a: any) => a.role === 'SPECIAL_APPROVER' && (a.approverId === currentUserId || a.status !== 'PENDING')))
        );

        setApprovals(
          myApprovals.map((s: any) => {
            const myApproval = s.approvals?.find((a: any) => a.role === 'SPECIAL_APPROVER');
            const approvalStatus: ApprovalFilter =
              myApproval?.status === 'APPROVED'  ? 'APPROVED'  :
              myApproval?.status === 'REJECTED'  ? 'CANCELLED' : 'PENDING';
            return {
              id: s.id, requestNo: s.submissionNo, formTitle: s.formName,
        ...
        // Pending workflows = approvals still needing action
        setWorkflows(
          myApprovals
        ...
        // Also add initiator resubmission workflows
