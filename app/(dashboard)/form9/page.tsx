...
import {
  ...
  CheckCircle2, FileText, Upload, X, Eye, Trash2,
  AlertCircle, Loader2, Calendar, ChevronDown, ArrowLeft,
} from 'lucide-react';
...
interface AttachedFile { id: string; label: string; file?: File; fileUrl?: string; size?: number }
...
function Form9Inner() {
  ...
  // Documents
  const [docFiles, setDocFiles] = useState<Record<string, AttachedFile | null>>({});
  ...
  const handleFileChange = (label: string, file: File | null) => {
    setDocFiles(prev => ({ ...prev, [label]: file ? { id: label, label, file, size: file.size } : null }));
  };

  const handleSubmitNew = async () => {
    if (!name || !companyCode || !contactNo || !gmcApprovalNo || !gmcApprovalDate ||
        !ownerType || !ownerName || !ownerContact || !assNo || propTypes.length === 0 ||
        !consideration || !planNo || !lotNo || facilities.length === 0 || !cdId || !gmcId) {
      setError('Please fill in all mandatory fields and select approvers.');
      return;
    }
  ...
  const handleSubmitDocs = async () => {
    ...
    try {
      ...
      const pendingDocs = submission.documents.filter(d => docFiles[d.id]?.file);
      for (const doc of pendingDocs) {
        const f = docFiles[doc.id]?.file;
        if (!f) continue;
        const fd = new FormData(); fd.append('file', f);
        const up = await fetch('/api/upload', { method: 'POST', body: fd });
        const ud = await up.json();
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          {isReadOnly
                            ...
                            : <div><FieldLabel text="GMC App. Date*" /><input type="date" value={gmcApprovalDate} onChange={e => setGmcApprovalDate(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#1A438A]/30" /></div>
                          }
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {isReadOnly
                          ...
                          : <div><FieldLabel text="Property Owner Type*" /><select value={ownerType} onChange={e => setOwnerType(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#1A438A]/30"><option value="">Select...</option>{['Company','Individual','Partnership'].map(t => <option key={t}>{t}</option>)}</select></div>
                        }
                        {isReadOnly
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        {isReadOnly
                          ...
                          : <div><FieldLabel text="Date of the COC (For Buildings)" /><input type="date" value={cocDate} onChange={e => setCocDate(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#1A438A]/30" /></div>
                        }
        ...
        <div className="w-[296px] flex-shrink-0 space-y-4">
                ...
                <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                  ...
                  <div className="p-3 space-y-1.5 min-h-[96px]">
                    {(mode === 'docs' ? (s?.documents || []) : INITIAL_DOCS.map(label => {
                      const doc = s?.documents.find(d => d.label === label);
                      return doc || { id: label, label, type: 'required', status: 'NONE', fileUrl: null };
                    })).map((doc: any, i: number) => {
                      const hasFile = !!(docFiles[doc.id]?.file || doc.fileUrl);
                      return (
                        <div key={doc.id || i}
                          ...
                          <div className="flex-1 mr-2 min-w-0">
                            ...
                            {docFiles[doc.id]?.file && <span className="text-[10px] text-emerald-600 font-semibold">1 file ready</span>}
                            {doc.fileUrl && !docFiles[doc.id]?.file && <span className="text-[10px] text-emerald-600 font-semibold">1 file attached</span>}
                          </div>
                          <div className="flex items-center gap-1 flex-shrink-0">
                            ...
                            {(mode === 'new' || mode === 'docs') ? (
                              <label className="flex-shrink-0 transition-colors cursor-pointer">
                                {hasFile
                                  ? <CheckCircle2 className="w-4 h-4 text-emerald-500 hover:text-emerald-600" />
                                  : <Upload className="w-4 h-4 text-[#1183B7] hover:text-[#1A438A]" />}
