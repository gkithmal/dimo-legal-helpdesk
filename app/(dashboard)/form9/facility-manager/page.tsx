...
function FMInner() {
    ...
    try {
      if (comment?.trim()) {
        await fetch(`/api/submissions/${submission.id}/comments`, {
          ...
          body: JSON.stringify({ authorName: userName, authorRole: 'FACILITY_MANAGER', text: comment }),
        });
      }
      await fetch(`/api/submissions/${submission.id}/approve`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ role: 'FACILITY_MANAGER', action, approverName: userName, comment }),
      });
      setShowSuccess(
        action === 'APPROVED'
          ? 'Site approval granted. The Legal Officer will proceed with deed drafting.'
  ...
  const alreadyActed = s && !['PENDING_FACILITY_MANAGER'].includes(s.status);
  ...
  return (
    <div className="min-h-screen flex bg-[#f0f4f9]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                ...
                <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-100">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-slate-400">
                      ...
                      <span className="ml-2 px-1.5 py-0.5 rounded bg-teal-100 text-teal-700 text-[10px] font-semibold">Facility Manager Review</span>
                    </p>
                  </div>
                </div>
                  ...
                  {s && (
                    <>
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                        <p className="text-xs text-amber-700 font-semibold">Site Visit Required</p>
                        <p className="text-xs text-amber-600 mt-0.5">
                          Please visit the premises at <span className="font-bold">{s.f9PremisesAssNo}</span> before approving. 
                          Your approval confirms the premises meets operational requirements for the purchase.
                        </p>
                      </div>
                    </>
                  )}
                ...
                {!alreadyActed && (
                  <>
                    ...
                    <button onClick={() => setConfirmModal('approve')} disabled={acting}
                      ...
                      {acting ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : 'Approve Premises'}
                    </button>
                  </>
                )}
              </div>
      ...
      {confirmModal === 'approve' && (
        <ConfirmModal title="Approve Premises?" message="Your site visit approval will be recorded. The Legal Officer will proceed with deed drafting."
...
export default function Form9FMPage() {
  return (
