...
import {
  ...
  ArrowLeft, CheckCircle2, FileText, Eye, Upload, Loader2,
} from 'lucide-react';
...
function GMCInner() {
  ...
  const [gmcDocFile, setGmcDocFile]     = useState<File | null>(null);
  ...
  const handleApprove = async (comment?: string) => {
    ...
    setActing(true);
    try {
      // Upload GMC approval document first if provided
      let gmcDocUrl: string | null = null;
      if (gmcDocFile) {
        setUploading(true);
        const fd = new FormData(); fd.append('file', gmcDocFile);
        ...
        if (ud.success) {
          ...
          await fetch(`/api/submissions/${submission.id}`, {
            ...
            body: JSON.stringify({ addDocument: { label: 'GMC Approval Document', type: 'legal', fileUrl: gmcDocUrl } }),
          });
        }
        setUploading(false);
      }
      ...
      if (comment?.trim()) {
        await fetch(`/api/submissions/${submission.id}/comments`, {
          ...
          body: JSON.stringify({ authorName: userName, authorRole: 'GMC_MEMBER', text: comment }),
        });
      }

      await fetch(`/api/submissions/${submission.id}/approve`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ role: 'GMC_MEMBER', action: 'APPROVED', approverName: userName, comment }),
      });

      setShowSuccess('GMC approval granted. The BUM will be notified to submit the required documents.');
    } finally { setActing(false); setUploading(false); }
  };
  ...
  const handleCancel = async (comment?: string) => {
    ...
    try {
      ...
      await fetch(`/api/submissions/${submission.id}/approve`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ role: 'GMC_MEMBER', action: 'CANCELLED', approverName: userName, comment }),
      });
      setShowSuccess('The request has been cancelled.');
  ...
  const alreadyActed = s && !['PENDING_GMC'].includes(s.status);
  ...
  return (
    <div className="min-h-screen flex bg-[#f0f4f9]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="max-w-[1200px] mx-auto px-6 py-6">
          <div className="flex gap-5">
                ...
                <div className="flex items-center gap-3 px-5 py-3 border-b border-slate-100">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-slate-400">
                      ...
                      <span className="ml-2 px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-700 text-[10px] font-semibold">GMC Review</span>
                    </p>
                  </div>
                </div>
            ...
            <div className="w-[280px] flex-shrink-0 space-y-4">
              ...
              {/* GMC Approval Document Upload */}
              {!alreadyActed && (
                <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-100">
                    <span className="text-[11px] font-bold uppercase tracking-widest text-[#17293E]">GMC Approval Document</span>
                  </div>
                  <div className="p-3">
                    <label className={`flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 border-dashed cursor-pointer transition-all
                      ${gmcDocFile ? 'border-emerald-400 bg-emerald-50' : 'border-slate-300 hover:border-[#1A438A] hover:bg-[#1A438A]/5'}`}>
                      {gmcDocFile
                        ? <><CheckCircle2 className="w-4 h-4 text-emerald-500" /><span className="text-xs font-semibold text-emerald-600 truncate max-w-[160px]">{gmcDocFile.name}</span></>
                        : <><Upload className="w-4 h-4 text-slate-400" /><span className="text-xs text-slate-500">Upload GMC Approval Doc</span></>
                      }
                      <input type="file" className="hidden" accept=".pdf,.doc,.docx,.jpg,.png"
                        onChange={e => setGmcDocFile(e.target.files?.[0] || null)} />
                    </label>
                    <p className="text-[10px] text-slate-400 mt-1.5 text-center">Upload as part of your approval (optional)</p>
                  </div>
                </div>
              )}
                ...
                {!alreadyActed && (
                  <>
                    <button onClick={() => setConfirmModal('cancel')} disabled={acting}
                      className="w-full py-2.5 rounded-xl text-white text-sm font-semibold bg-red-500 hover:bg-red-600 disabled:opacity-50">
                      Cancel Request
                    </button>
                    <button onClick={() => setConfirmModal('approve')} disabled={acting || uploading}
                      ...
                      {acting || uploading ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : 'Approve & Notify BUM'}
                    </button>
                  </>
                )}
              </div>
      ...
      {confirmModal === 'approve' && (
        <ConfirmModal title="Approve Request?" message="Your GMC approval will be recorded and the BUM will be notified to submit the required documents."
          confirmLabel="Yes, Approve" confirmClass="bg-[#89BD3B] hover:bg-[#7aaa30]" requireComment
          onConfirm={(c) => { setConfirmModal(null); handleApprove(c); }} onClose={() => setConfirmModal(null)} />
      )}
      {confirmModal === 'cancel' && (
        <ConfirmModal title="Cancel Request?" message="This action is irreversible. The request will be permanently cancelled."
          ...
          onConfirm={(c) => { setConfirmModal(null); handleCancel(c); }} onClose={() => setConfirmModal(null)} />
      )}
...
export default function Form9GMCPage() {
  return (
