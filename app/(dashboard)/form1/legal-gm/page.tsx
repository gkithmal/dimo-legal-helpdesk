...
function LegalGMPageContent() {
  ...
  const [hasActed, setHasActed] = useState(false); // local optimistic flag
  ...
  const loadSubmission = useCallback(async () => {
    ...
    try {
      ...
      setSubmission(s);
      // s.assignedLegalOfficer = raw user ID; s.legalOfficerName = resolved display name
      const officerName = s.legalOfficerName || s.assignedLegalOfficer || '';
      const officerId = s.assignedLegalOfficer || '';
      setAssignedOfficer({ name: officerName, email: '', id: officerId });
  ...
  useEffect(() => {
    fetch('/api/users?role=LEGAL_OFFICER')
      .then(r => r.json())
      .then(d => {
        if (d.success) setLegalOfficers(d.data.map((u: any) => ({ id: u.id, name: u.name || u.email, email: u.email })));
      })
      .catch(console.error);
  }, []);
  ...
  const gmAlreadyActed = hasActed || (!!submission && !['PENDING_LEGAL_GM', 'PENDING_LEGAL_GM_FINAL'].includes(submission.status));
  const activeStep = (() => {
  ...
  setAssignedOfficer({ name: n, email: e, id: officerId });
  if (submissionId) {
