...
// LO has multiple stages in Form 10
...
// ─── Official Use Modal (Form 10 LOD finalization fields) ──────────────────────────
type OfficialFields = {
  gmcApprovalNo: string; caseNo: string; caseFillingDate: string;
  council: string; court: string; remarks: string;
  lodRefNo: string; lodSentDate: string; lodRemarks: string;
};
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Letter of Demand Reference No</label>
              <input value={fields.lodRefNo||''} onChange={e => set('lodRefNo', e.target.value)}
            ...
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Postal Article (PDF)</label>
              <input type="file" accept="application/pdf"
                className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:border-[#1A438A]" />
            </div>
            <div className="col-span-2">
              <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Remarks</label>
              <input value={fields.lodRemarks||''} onChange={e => set('lodRemarks', e.target.value)}
                className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 bg-slate-50 text-sm focus:outline-none focus:border-[#1A438A]" />
            </div>
          </div>
        </div>
...
function LegalOfficerForm3PageContent() {
  ...
  const handleSaveOfficialUse = async (fields: OfficialFields) => {
    ...
    await fetch(`/api/submissions/${submissionId}`, {
      ...
      body: JSON.stringify({ f3GmcApprovalNo: fields.lodRefNo, f3CaseNo: fields.lodSentDate, f3CaseFillingDate: fields.caseFillingDate, f3Council: fields.council, f3Court: fields.court, f3Remarks: fields.lodRemarks }),
    });
  };
    ...
    <div className="min-h-screen flex bg-[#f0f4f9]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <div className="rounded-2xl overflow-hidden shadow-sm" style={{ background: 'linear-gradient(135deg, #1A438A 0%, #1e5aad 100%)' }}>
            <div className="flex items-center justify-between px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-white font-bold text-base">Instruction to Issue Letter of Demand</h1>
                  <p className="text-white/50 text-[11px] mt-0.5 font-mono">16/FM/1641/07/10</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
      ...
      {showOfficialUse && (
        <OfficialUseModal
          ...
          initialFields={{ gmcApprovalNo: '', caseNo: '', caseFillingDate: '', council: '', court: '', remarks: '', lodRefNo: submission.f3GmcApprovalNo || '', lodSentDate: submission.f3CaseNo || '', lodRemarks: submission.f3Remarks || '' }}
