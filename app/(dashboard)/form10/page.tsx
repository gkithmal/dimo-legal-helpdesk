...
const FORM10_DOCS_BY_TYPE: Record<string, string[]> = {
...
function CustomerSection({ customerType, data, onChange, disabled, submitted }: {
  ...
  if (customerType === 'Individual') return (
    <div className="space-y-4">
      ...
      <div><FieldLabel required>NIC No</FieldLabel><TextField value={data.nicNo||''} onChange={v=>onChange('nicNo',v)} disabled={disabled} hasError={!!err('nicNo','NIC')}/><FieldError message={err('nicNo','NIC')}/></div>
      <div><FieldLabel required>Residence Address</FieldLabel><TextField value={data.residentialAddress||''} onChange={v=>onChange('residentialAddress',v)} disabled={disabled} hasError={!!err('residentialAddress','Residence Address')}/><FieldError message={err('residentialAddress','Residence Address')}/></div>
  ...
  if (customerType === 'Sole-proprietorship') return (
    <div className="space-y-4">
      ...
      <div><FieldLabel required>Principle Place of Business</FieldLabel><TextField value={data.principalPlaceOfBusiness||''} onChange={v=>onChange('principalPlaceOfBusiness',v)} disabled={disabled} hasError={!!err('principalPlaceOfBusiness','Principle Place of Business')}/><FieldError message={err('principalPlaceOfBusiness','Principle Place of Business')}/></div>
      <div className="grid grid-cols-2 gap-4">
        <div><FieldLabel required>Contact No</FieldLabel><TextField value={data.contactNo||''} onChange={v=>onChange('contactNo',v)} disabled={disabled} hasError={!!err('contactNo','Contact No')}/><FieldError message={err('contactNo','Contact No')}/></div>
        <div><FieldLabel required>Email Address</FieldLabel><TextField value={data.emailAddress||''} onChange={v=>onChange('emailAddress',v)} type="email" disabled={disabled} hasError={!!err('emailAddress','Email')}/><FieldError message={err('emailAddress','Email')}/></div>
      </div>
  ...
  if (customerType === 'Partnership') return (
    <div className="space-y-4">
      ...
      <div><FieldLabel required>Principle Place of Business</FieldLabel><TextField value={data.principalPlaceOfBusiness||''} onChange={v=>onChange('principalPlaceOfBusiness',v)} disabled={disabled} hasError={!!err('principalPlaceOfBusiness','Principle Place of Business')}/><FieldError message={err('principalPlaceOfBusiness','Principle Place of Business')}/></div>
      <div>
        <FieldLabel required>Name of the Owners</FieldLabel>
...
function Form10PageContent() {
  ...
  const buildMeta = () => JSON.stringify({
    initiatorName, initiatorContact,
  ...
  const handleSubmitClick = async (asDraft = false) => {
    ...
    try {
      const payload = {
        formId: 10, formName: 'Instruction to Issue Letter of Demand',
        status: asDraft ? 'DRAFT' : 'PENDING_APPROVAL',
        initiatorId: session?.user?.id || '',
        companyCode, title: 'Instruction to Issue Letter of Demand',
        sapCostCenter, scopeOfAgreement: buildMeta(),
        term: '', value: customerData.outstandingAmount||'0',
      ...
      const res = await fetch(isDraftEdit ? `/api/submissions/${submissionId}` : '/api/submissions', {
        ...
        body: JSON.stringify(isDraftEdit ? { status: asDraft?'DRAFT':'PENDING_APPROVAL', companyCode, sapCostCenter, scopeOfAgreement: buildMeta(), value: customerData.outstandingAmount||'0' } : payload),
      });
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          ...
          <div className="rounded-2xl overflow-hidden shadow-sm" style={{background:'linear-gradient(135deg, #1A438A 0%, #1e5aad 100%)'}}>
            <div className="flex items-center justify-between px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-white font-bold text-base leading-tight">Instruction to Issue Letter of Demand</h1>
                  <p className="text-white/50 text-[11px] mt-0.5 font-mono tracking-wide">16/FM/1641/07/10</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                {mode!=='new' && (
                ...
                <div className="bg-white text-[#1A438A] font-bold text-sm px-4 py-2 rounded-xl shadow-lg shadow-black/10">Form 10</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm">
            <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100">
              ...
              <span className="text-[11px] font-bold uppercase tracking-widest text-[#17293E]">Form Details</span>
            </div>
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 flex flex-col items-center text-center">
            ...
            <p className="text-slate-500 text-sm mb-4 leading-relaxed">Your Letter of Demand request has been submitted for approval by BUM and FBP.</p>
