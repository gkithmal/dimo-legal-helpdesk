...
type RequiredDoc = { id:string; label:string; type:string; isRequired:boolean; };
...
function AddDocModal({onAdd,onClose}:{onAdd:(label:string,type:string,isRequired:boolean)=>void;onClose:()=>void}){
  const [label,setLabel]=useState('');const [type,setType]=useState('Common');const [isRequired,setIsRequired]=useState(true);
  return(<div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onClose}/><div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8"><h2 className="text-[#17293E] font-bold text-base mb-6">Enter the Document Name</h2><textarea value={label} onChange={(e)=>setLabel(e.target.value)} rows={3} className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-[#1A438A] resize-none mb-5" placeholder="Enter document name..."/><div className="mb-4"><label className="block text-sm font-semibold text-slate-600 mb-2">Select the Type</label><div className="relative"><select value={type} onChange={(e)=>setType(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border-2 border-slate-200 text-sm focus:outline-none appearance-none bg-white">{DOC_TYPES.map((t)=><option key={t}>{t}</option>)}</select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none"/></div></div><div className="flex items-center justify-between px-4 py-3 rounded-xl border-2 border-slate-200 mb-6"><div><p className="text-sm font-semibold text-slate-700">Mandatory in initial submission</p><p className="text-[11px] text-slate-400">Initiator must attach this document before submitting</p></div><button type="button" onClick={()=>setIsRequired(!isRequired)} className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 ${isRequired?'bg-[#1A438A]':'bg-slate-300'}`}><div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${isRequired?'translate-x-5':'translate-x-0.5'}`}/></button></div><div className="flex gap-3"><button onClick={onClose} className="flex-1 py-3 rounded-xl font-bold text-sm border-2 border-slate-200 text-slate-600">Cancel</button><button disabled={!label.trim()} onClick={()=>{onAdd(label.trim(),type,isRequired);onClose();}} className="flex-1 py-3 rounded-xl font-bold text-sm text-white disabled:opacity-40" style={{background:'linear-gradient(135deg, #1A438A, #1e5aad)'}}>OK</button></div></div></div>);
}
function EditDocModal({doc,onSave,onClose}:{doc:RequiredDoc;onSave:(label:string,type:string,isRequired:boolean)=>void;onClose:()=>void}){
  const [label,setLabel]=useState(doc.label);const [type,setType]=useState(doc.type);const [isRequired,setIsRequired]=useState(doc.isRequired);
  return(<div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onClose}/><div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8"><h2 className="text-[#17293E] font-bold text-base mb-6">Edit Document</h2><textarea value={label} onChange={(e)=>setLabel(e.target.value)} rows={3} className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 text-sm focus:outline-none focus:border-[#1A438A] resize-none mb-5"/><div className="mb-4"><label className="block text-sm font-semibold text-slate-600 mb-2">Type</label><div className="relative"><select value={type} onChange={(e)=>setType(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border-2 border-slate-200 text-sm focus:outline-none appearance-none bg-white">{DOC_TYPES.map((t)=><option key={t}>{t}</option>)}</select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none"/></div></div><div className="flex items-center justify-between px-4 py-3 rounded-xl border-2 border-slate-200 mb-6"><div><p className="text-sm font-semibold text-slate-700">Mandatory in initial submission</p><p className="text-[11px] text-slate-400">Initiator must attach this document before submitting</p></div><button type="button" onClick={()=>setIsRequired(!isRequired)} className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 ${isRequired?'bg-[#1A438A]':'bg-slate-300'}`}><div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${isRequired?'translate-x-5':'translate-x-0.5'}`}/></button></div><div className="flex gap-3"><button onClick={onClose} className="flex-1 py-3 rounded-xl font-bold text-sm border-2 border-slate-200 text-slate-600">Cancel</button><button disabled={!label.trim()} onClick={()=>{onSave(label.trim(),type,isRequired);onClose();}} className="flex-1 py-3 rounded-xl font-bold text-sm text-white disabled:opacity-40" style={{background:'linear-gradient(135deg, #AC9C2F, #c9b535)'}}>Save</button></div></div></div>);
}
...
function FormSettingsTab() {
  ...
  const addDoc=(label:string,type:string,isRequired:boolean)=>updateConfig((c)=>({...c,docs:[...c.docs,{id:`d_${Date.now()}`,label,type,isRequired}]}));
  const editDoc=(id:string,label:string,type:string,isRequired:boolean)=>updateConfig((c)=>({...c,docs:c.docs.map((d)=>d.id===id?{...d,label,type,isRequired}:d)}));
  ...
  const saveChanges=async()=>{
    ...
    try{
      const res=await fetch('/api/settings/forms',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({formId:config.formId,instructions:config.instructions,docs:config.docs.map((d)=>({label:d.label,type:d.type,isRequired:d.isRequired}))})});
      const data=await res.json();
      if(!res.ok||!data.success)throw new Error(data.error||'Save failed');
      setShowSaved(true);
    }catch(err:unknown){setApiError(err instanceof Error?err.message:'Save failed');}
  ...
  return(
    <div className="flex gap-5 flex-1 min-h-0">
              ...
              {!config||config.docs.length===0
                ...
                :config.docs.map((doc,i)=>(
                  <div key={doc.id} className="flex items-center gap-2 px-3 py-2.5 hover:bg-slate-50 group">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm text-slate-700 truncate">{doc.label}</p><div className="flex items-center gap-1.5 mt-0.5"><span className="text-[10px] text-slate-400 font-medium">{doc.type}</span><span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded ${doc.isRequired?'bg-red-50 text-red-500':'bg-slate-100 text-slate-400'}`}>{doc.isRequired?'Required':'Optional'}</span></div></div>
      ...
      {editingDoc&&<EditDocModal doc={editingDoc} onSave={(label,type,isRequired)=>{editDoc(editingDoc.id,label,type,isRequired);setEditingDoc(null);}} onClose={()=>setEditingDoc(null)}/>}
