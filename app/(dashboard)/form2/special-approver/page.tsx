...
import {
  ...
  ArrowLeft, CheckCircle2, FileText, Clock,
...
// ─── Main Page ────────────────────────────────────────────────────────────────
function SpecialApproverForm2Content() {
  ...
  // ── Approval details from loaded data ──
  ...
  const [lessorParties,            setLessorParties]            = useState<{type:string;name:string}[]>([]);
  ...
  const [intendToConstruct,        setIntendToConstruct]        = useState('');
  ...
  const loadSubmission = useCallback(async () => {
    ...
    try {
      ...
      setLessorParties(meta.lessorParties || []);
      setNicNo(meta.nicNo || '');
      ...
      setAssetHouse(!!meta.assetHouse);
      setAssetLand(!!meta.assetLand);
      setAssetBuilding(!!meta.assetBuilding);
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm">
            ...
            <div className="px-6 py-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <ReadField label="Dept. SAP Code" value={deptSapCode} />
              </div>
              <ReadField label="Purpose of Lease" value={purposeOfLease} />

              <SectionDivider>Property Owner (Lessor)</SectionDivider>

              {/* Lessor Parties Table */}
              <div>
                <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Lessor Parties</label>
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  ...
                  {lessorParties.filter(p => p.type || p.name).length === 0
                    ? <div className="px-3.5 py-3 text-sm text-slate-400 italic">—</div>
                    : lessorParties.filter(p => p.type || p.name).map((p, i) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <ReadField label="VAT Reg. No." value={vatRegNo} />
              </div>
              <ReadField label="Contact (Lessor)" value={lessorContact} />
              <ReadField label="Name of Lessee / Tenant" value={leaseName} />

              <SectionDivider>Lease Terms</SectionDivider>

              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Premises Bearing Asst. No" value={premisesAssetNo} />
                <ReadField label="Period of Lease" value={periodOfLease} />
              </div>

              <div>
                <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">Asset Type</label>
                <div className="flex items-center gap-3 flex-wrap px-1">
                  {assetHouse    && <span className="px-3 py-1 rounded-full bg-[#EEF3F8] text-[#1A438A] text-xs font-bold">House</span>}
                  {assetLand     && <span className="px-3 py-1 rounded-full bg-[#EEF3F8] text-[#1A438A] text-xs font-bold">Land</span>}
                  {assetBuilding && <span className="px-3 py-1 rounded-full bg-[#EEF3F8] text-[#1A438A] text-xs font-bold">Building</span>}
                  {!assetHouse && !assetLand && !assetBuilding && <span className="text-sm text-slate-400 italic">—</span>}
                  {assetExtent && <span className="text-sm text-slate-500 ml-2">Extent: {assetExtent}</span>}
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Commencing From" value={commencingFrom} />
                <ReadField label="Ending On" value={endingOn} />
              </div>
              <ReadField label="Monthly Rental Rs." value={monthlyRental} />
              <div className="grid grid-cols-3 gap-4">
                <ReadField label="Advance Payment Rs." value={advancePayment} />
                <ReadField label="Deductible Rate Rs." value={deductibleRate} />
                <ReadField label="Period" value={deductiblePeriod} />
              </div>
              <ReadField label="Refundable Deposit Rs." value={refundableDeposit} />
              <ReadField label="Electricity, Water & Phone" value={electricityWaterPhone} />

              <SectionDivider>Renewal Details</SectionDivider>

              <ReadField label="If a Renewal, Previous Agreement No" value={previousAgreementNo} />
              <ReadField label="Date of the Principal Agreement" value={dateOfPrincipalAgreement} />
              <div className="grid grid-cols-2 gap-4">
                <ReadField label="Buildings Fully / Partly Constructed?" value={buildingsConstructed} />
                <ReadField label="Intend to Construct?" value={intendToConstruct} />
              </div>
              <ReadField label="Remarks" value={remarks} />
            </div>
          </div>
        </div>
        ...
        <div className="w-[296px] flex-shrink-0 flex flex-col gap-4">
          ...
          <PanelSection title="Approvals">
            <div className="p-4 space-y-3.5">
              {[{ label: 'BUM', value: bum }, { label: 'FBP', value: fbp }, { label: 'Cluster Head', value: clusterHead }].map(({ label, value }) => (
                <div key={label}>
                  <label className="block text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-1.5">{label}</label>
...
export default function SpecialApproverForm2Page() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="text-gray-600">Loading...</div></div>}>
      <SpecialApproverForm2Content />
    </Suspense>
  );
}